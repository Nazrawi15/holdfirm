...
export function UserStats({ address }: UserStatsProps) {
  ...
  return (
    <div>
      ...
      {performance && (
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px', marginBottom: '20px' }}>
          <div style={{ backgroundColor: '#f0fdf4', border: '1px solid #bbf7d0', borderRadius: '14px', padding: '16px' }}>
            <p style={{ color: '#9ca3af', fontSize: '11px', fontWeight: 600, letterSpacing: '0.5px', margin: '0 0 4px 0' }}>
              YIELD EARNED (REALIZED)
            </p>
      ...
      {history.length > 0 && (
        <div>
          <p style={{ color: '#9ca3af', fontSize: '11px', fontWeight: 600, letterSpacing: '0.5px', margin: '0 0 10px 0' }}>
            RECENT TRANSACTIONS (VIA SDK)
          </p>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
            {history.map((tx, i) => (
              <div key={i} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', backgroundColor: '#f9fafb', border: '1px solid #f3f4f6', borderRadius: '10px', padding: '12px 16px' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                  <span style={{ fontSize: '18px' }}>{tx.type === 'deposit' ? '↑' : '↓'}</span>
                  <div>
                    <p style={{ color: '#111827', fontWeight: 700, fontSize: '14px', margin: 0, textTransform: 'capitalize' }}>{tx.type}</p>
                ...
                <div style={{ textAlign: 'right' }}>
                  ...
                  <a href={`https://basescan.org/tx/${tx.transactionHash}`} target="_blank" rel="noopener noreferrer" style={{ color: '#9ca3af', fontSize: '11px' }}>
                    View →
                  </a>
                </div>
              </div>
      ...
      {!loading && !performance && history.length === 0 && (
        <div style={{ backgroundColor: '#f9fafb', border: '1px solid #f3f4f6', borderRadius: '12px', padding: '24px', textAlign: 'center' }}>
